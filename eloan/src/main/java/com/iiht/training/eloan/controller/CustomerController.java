...
public class CustomerController {
	...
	public ResponseEntity<UserDto> register(@Valid @RequestBody UserDto userDto, BindingResult result){
		...
		if(result.hasErrors()) {
			throw new CustomerNotFoundException("Invalid data format! Cannot create Customer");
		}
	...
	public ResponseEntity<LoanOutputDto> applyLoan(@PathVariable Long customerId,
		...
		if(result.hasErrors()) {
			throw new CustomerNotFoundException("Invalid data format! Failed to Apply for Loan");
		}
		
		LoanOutputDto outputDto = new LoanOutputDto();
		outputDto = this.customerService.applyLoan(customerId, loanDto);
		ResponseEntity<LoanOutputDto> response = new ResponseEntity<LoanOutputDto> (outputDto, HttpStatus.OK);
	...
	public ResponseEntity<List<LoanOutputDto>> getStatusAll(@PathVariable Long customerId){
		// call the service layer method
				List<LoanOutputDto> loanOutputDtos = this.customerService.getStatusAll(customerId);
